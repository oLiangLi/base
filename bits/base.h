...
#define rLANG_VERSION_MINOR 12
#define rLANG_VERSION_RELEASE 120
...
#if defined(__cplusplus) && !defined(rLANG_CONFIG_MINIMAL)
#include <string>
#endif /* __cplusplus && !rLANG_CONFIG_MINIMAL */
...
/**
 *! ASSERT(rLANGAPI == cdecl)
 */
#if defined(__i386__)
#error "rLANGAPI != cdecl, I don't think anyone still uses i386 programs"
#endif /* defined(__i386__) */
#if 0  /* ASSERT(rLANGAPI == cdecl) */
...
#if !defined(rLANG_DLLEXPORT) && defined(_WIN32)
#define rLANG_DLLEXPORT __declspec(dllexport)
#elif !defined(rLANG_DLLEXPORT)
#define rLANG_DLLEXPORT __attribute__((visibility("default")))
#endif /* rLANG_DLLEXPORT */

#if !defined(rLANG_DLLIMPORT) && defined(_WIN32)
#define rLANG_DLLIMPORT __declspec(dllimport)
#elif !defined(rLANG_DLLIMPORT)
#define rLANG_DLLIMPORT __attribute__((visibility("default")))
#endif /* rLANG_DLLIMPORT */
...
rlBASE_INLINE rlLogLevel rlLoggingGetLevel() {
  return rlLoggingSetLevel(rlLOG_FATAL); /* invalid argument : level < rlLOG_ERROR */
}
...
#define rlLOGX(tag, fmt, ...) rlLoggingWrite(rlLOG_FATAL, (tag), __LINE__, (fmt), ##__VA_ARGS__)
#define rlWriteLog(level, tag, fmt, ...) rlLoggingWrite((level), (tag), __LINE__, (fmt), ##__VA_ARGS__)
...
#define rlLOGXX(tag, dat, len, fmt, ...) \
  rlLoggingWriteEx(rlLOG_FATAL, (tag), __LINE__, (dat), (int)(len), (fmt), ##__VA_ARGS__)
#define rlWriteExLog(level, tag, dat, len, fmt, ...) \
  rlLoggingWriteEx((level), (tag), __LINE__, (dat), (int)(len), (fmt), ##__VA_ARGS__)
#endif /* rlLOGV */
...
rlBASECC_INLINE uint32_t Xs(const char* magic) {
  return rLANG_DECLARE_MAGIC_Xs(magic);
}

rlBASECC_INLINE const char* Vs(uint32_t magic, char buffer[6]) {
  return rLANG_DECLARE_MAGIC_Vs(magic, buffer);
}

#ifndef rLANG_CONFIG_MINIMAL
rlBASE_INLINE std::string Vs(uint32_t magic) {
  char buffer[6] = {
      (char)rLANG_DECLARE_MAGIC_Vc(magic >> (26 - 6 * 0)), (char)rLANG_DECLARE_MAGIC_Vc(magic >> (26 - 6 * 1)),
      (char)rLANG_DECLARE_MAGIC_Vc(magic >> (26 - 6 * 2)), (char)rLANG_DECLARE_MAGIC_Vc(magic >> (26 - 6 * 3)),
      (char)rLANG_DECLARE_MAGIC_Vc(magic >> (26 - 6 * 4))};
  return buffer;
}
#endif /* rLANG_CONFIG_MINIMAL */
}  // namespace Magic
...
class CryptoSHA1 {
 public:
  ...
  CryptoSHA1& Init() {
    rlCryptoSha1CtxInit(&ctx_);
    return *this;
  }
  CryptoSHA1& Update(const void* buffer, int size) {
    rlCryptoSha1CtxUpdate(&ctx_, buffer, size);
    return *this;
  }
  CryptoSHA1& Final(void* md) {
    rlCryptoSha1CtxFinal(&ctx_, (uint8_t*)md);
    return *this;
  }
  CryptoSHA1& Clear() {
...
class CryptoSHA256 {
 public:
  ...
  CryptoSHA256& Update(const void* buffer, int size) {
    rlCryptoSha256CtxUpdate(&ctx_, buffer, size);
    return *this;
  }
  CryptoSHA256& Final(void* md) {
    rlCryptoSha256CtxFinal(&ctx_, (uint8_t*)md);
    return *this;
  }
  CryptoSHA256& Clear() {
    memset(&ctx_, 0, sizeof(ctx_));
...
class CryptoSHA384 {
 public:
  enum { DIGEST_LENGTH = 48 };

  CryptoSHA384& Init() {
    rlCryptoSha384CtxInit(&ctx_);
    return *this;
  }
  CryptoSHA384& Update(const void* buffer, int size) {
    rlCryptoSha384CtxUpdate(&ctx_, buffer, size);
    return *this;
  }
  CryptoSHA384& Final(void* md) {
    rlCryptoSha384CtxFinal(&ctx_, (uint8_t*)md);
    return *this;
  }
  CryptoSHA384& Clear() {
    memset(&ctx_, 0, sizeof(ctx_));
...
class CryptoSHA512 {
 public:
  ...
  CryptoSHA512& Update(const void* buffer, int size) {
    rlCryptoSha512CtxUpdate(&ctx_, buffer, size);
    return *this;
  }
  CryptoSHA512& Final(void* md) {
    rlCryptoSha512CtxFinal(&ctx_, (uint8_t*)md);
    return *this;
  }
  CryptoSHA512& Clear() {
    memset(&ctx_, 0, sizeof(ctx_));
